...
// From https://github.com/cameron314/concurrentqueue/blob/master/blockingconcurrentqueue.h
...
namespace moodycamel
{
    ...
    public:
        ...
        BlockingConcurrentQueue(size_t minCapacity,
            ...
            : inner(minCapacity, maxExplicitProducers, maxImplicitProducers), sema(create<LightweightSemaphore, ssize_t, int>(0, (int)Traits::MAX_SEMA_SPINS),
                                                                                   &BlockingConcurrentQueue::template destroy<LightweightSemaphore>)
        {
            assert(reinterpret_cast<ConcurrentQueue *>((BlockingConcurrentQueue *)1) == &((BlockingConcurrentQueue *)1)->inner && "BlockingConcurrentQueue must have ConcurrentQueue as its first member");
            if (!sema)
    ...
    public:
        ...
        {
            return wait_dequeue_timed(token,
                                      item,
                                      std::chrono::duration_cast<std::chrono::microseconds>(timeout)
                                          .count());
        }
