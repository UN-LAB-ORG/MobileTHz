% Collection: https://www.nature.com/collections/hbfcgcbggg
...
% Q0-Vitaly: As we are moving to pre-ready article, please, check that it meets all the guidelines, e.g., not not exclusively here: https://www.nature.com/npjwireltech/for-authors-and-referees/submission-guidelines We'd also need a cover letter. Sergey, can you, please, draft one and put in this Overleaf project?
% AO-Sergey: 

% Q1-Vitaly: If we keep 7G, we'd need to modify the abstract and a bit of Intro accordingly. I can do this as needed.
% A1-Sergey: I think 'beyond 6G' is better, saying specifically '7G' is too specific for something that isn't guarenteed for 7G

% Q2-Vitaly: The more I think, the more the title "PanicPacket" for the proposed super-cool-solution sounds weak. Thoughts/Alternatives here? Should we call it "Just-in-time (JIT)", for instance? AKa the title
% A2-Sergey: I have removed it as Nature advises not to have any 'non-professional' names. PanicPacket -> Control Frames and algo is JIT.

% Q3-Vitaly: Sergey, please, fix all \cite{???} by using a proper citation to prove the point - from already existing citations, if applicable, but also new works, as needed.
% A3-Sergey: This is done.

% Q4-Vitaly: Sergey, please, also run spell-check one more time and proofread yourself before the submission
% A4-Sergey: This is done, but always a WIP.

% Q5-Vitaly: Illustration is NEEDED here! A redesign of earlier Figure 1.
% Q5-Sergey: WIP

% Q6-Vitaly: Let's carefully cross-check that all the KPIs are always used consistently across the paper (e.g., channel capacity vs capacity vs. data rate vs. rate, etc.) in every block and every figure.
% A6-Sergey: The main KPIs are:
% Mean Capacity (C̄)
% Capacity Coefficient of Variation (CoV)
% Link Availability (A_link)
% Signaling Overhead (O_sig)
% I have removed all mentions of 'stability' as it is ambiguous and replaced with more mathematical terminology

% Q7-Vitaly: I don't like HIL - what is this?]
% A7-Sergey: Hardware-in-the-Loop (HIL) "a technique used to develop and test embedded systems by connecting physical controller hardware to a real-time, simulated environment (digital twin) of the physical plant"

% Q8-Sergey: The list of references needs to be properly cleaned. Sergey, please, have a closer look into unifying everything as per the style guidelines and also putting in missing items (e.g., 6g -> 6G, mimo -> MIMO, etc.). Please, ask a pickly colleague (e.g., Sherif or Dbo) to have a look over and help you polish it further once you are done. Sounds minor, but the reviewers WILL pay attention to the accuracy we have there.
% A8-Sergey: Will ask Dbo

% Q9-Vitaly: Let's unify the use of Figure~1 vs. Fig.~1 in the text. Please, check the journal guidelines and prior papers in NPJ Wireless TEch (if any available) for which way to use best. But in any case - let's only use one way.
% A9-Sergey: I changed to Fig. notation, hard to find but last Nature had this http://mtscommon.nature.com/letters/author_style_checklist.pdf and also officially uses Fig. notation. Also they don't like to start sentences with Fig... so that has been changed

% Q10-Vitaly: re-alignment vs. realignment (check which one is better to use grammatically and use one all the time everywhere).
% A10-Sergey: Realignment is the proper term. Fixed.

% Q11-Vitaly: - we either call JIT a proactive scheme or a hybrid proactive+reactive scheme, but consistently everywhere.
% A11-Sergey: Go to question 2.

% Q12-Vitaly: Any public (e.g., Github?) source for MobileTHz platform? No need to clean the code much. We just keep bragging about it in multiple places, nice to back up the claim if we can. That would help in the review phase as well.
...
% A12-Sergey: I definitely have a Github repository for MobileTHz platform, I can cleanup code and we can publish it.

% Q12-Vitaly: I don't like " Blind Search" term. It sounds dumm and implies we compare our advanced algorithm against really low-quality solution. Not the impression one wants. It is not that blind honestly if one looks in Methods. "Hierarchical Search (HS)"? "Rotation-based search"? "Incremental Search"? or whater else comes to mind here.
% A12-Sergey: I like Hierarchical Search. 

% Q13-Vitaly: THz -> Sub-THz - that is what we need to decide for the entire paper! Title to conclusions. Let's use only one.
% A13-Sergey: Sub-THz is most appropriate

% Q14-Sergey: Appendix or Supplementary Information? I changed it to Supplementary.

% Q15-Sergey: Added a " statistics and reproducibility section " as per guidelines

% Styling https://www.nature.com/documents/npj-gta.pdf
\begin{abstract}
...
\end{abstract}
% The abstract must be self contained (all acronyms used must be defined)
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\linewidth]{Figures/vision_PanicPacket/v2.pdf}
\vspace{-8 pt}
...
\begin{figure*}[ht!]
    ...
    \begin{subfigure}[t]{0.475\textwidth}
        \vspace{0pt}
        \centering
        \input{Figures/Results/exampleMotion/exampleMotion}
        \vspace{-20pt}
    ...
    \begin{minipage}[t]{0.475\textwidth}
        ...
        \begin{subfigure}{\linewidth}
            \input{Figures/Results/translationalAcceleration/fast_CoV}
            \vspace{-15pt}
        ...
        \vfill
            \begin{subfigure}{\linewidth}
            \input{Figures/Results/translationalAcceleration/fast_overhead}
            \vspace{-15pt}
...
\begin{wrapfigure}{R}{0.4\linewidth}
    \centering
    \vspace{-15px}
    \includegraphics[width=\linewidth]{Figures/Methods/algorithm_baseline/baseline_v2_1.pdf}
    \begin{tikzpicture}
    \node [
            draw=black,
            line width=0.25pt,
...
\begin{wrapfigure}{r}{0.4\linewidth}
    \centering
    \vspace{-15px}
    \includegraphics[width=\linewidth]{Figures/Methods/algorithm_IMUreact/imu_v2.pdf}\\[-70pt]
    \begin{tikzpicture}
    \node [
            draw=black,
            line width=0.25pt,
...
\end{equation}
Unlike the expanding step size $HS_k$ used in the Hierarchical Search's Search Phase, the base refinement step $\beta \cdot \theta_{\text{HPBW}}$ is fixed and deliberately small (one-tenth of the beamwidth), while the IMU-predicted azimuth and elevation corrections adapt $IA_k$ over time as the sensor estimates evolve. The node scans a tight spiral around the IMU-predicted direction using this step size until an alignment is made.
...
\end{equation}
where $\boldsymbol{\omega}_{\text{UE}}$ is the UE's angular velocity, $\mathbf{v}_{\perp}$ is its translational velocity perpendicular to the LoS, and $d_{\text{link}}$ is the instantaneous link distance. This metric hence serves as measure of ``how fast the UE movements are approaching the AP beam boundaries''.
...
Code is publicly available at \url{https://doi.org/10.5281/zenodo.18675630}. All data can be made available to readers upon reasonable request to the corresponding author. 
...
\subsection*{Establishing Boundaries for IMU Sensor Data}
To establish realistic parameters for the mobility scenarios used in this paper, we conducted two experiments to capture typical human motion profiles. An IMU was affixed to a smartphone case to record its motion during common user activities. The first experiment (Fig.~\ref{fig:imu_hand_motion}) captures a phone-raising gesture, consisting of lifting the device from rest to ear level to simulate answering a phone call and then lowering it back down, repeated twice. This hybrid motion combines both translational and rotational components, producing translational accelerations exceeding \SI{10}{m/s^2} and rotational velocities up to \SI{200}{\degree/s}.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.99\linewidth}
        \centering
        \include{Figures/Appendix/IMU/raisingHandtoFace_smoothed_angular_acceleration}
        \vspace{-30pt}
...
The second experiment (Fig.~\ref{fig:imu_body_rotation}) isolates rotational-dominant motion by capturing a \SI{90}{\degree} body turn followed by a return to the original orientation, repeated twice. Here, rotational velocities are comparable to those observed in the first experiment, but translational accelerations remain low, driven primarily by centripetal forces. 
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.99\linewidth}
        \centering
        \include{Figures/Appendix/IMU/turningBody_smoothed_angular_acceleration.tex}
        \vspace{-30pt}
        \caption{Rotational acceleration.}
...
These measured profiles directly informed the evaluation ranges in the main study. The combined-motion sweep (Fig.~\ref{fig:antenna_comparison}) spans up to \SI{5}{m/s^2} and \SI{200}{deg/s^2}, matching the upper range observed here, while the single-axis sweeps in Figs.~\ref{fig:translational_rate} and~\ref{fig:rotational_rate} extend beyond the measured peaks to stress-test performance under extreme conditions. The ``Typical acceleration'' annotations in Figs.~\ref{fig:trans_sim_rate} and~\ref{fig:rot_sim_rate} mark the human-scale region captured by these experiments.
