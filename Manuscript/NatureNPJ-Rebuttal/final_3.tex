...
% Revision-marking macros: clean (final_2) version — render as identity.
% (final_revision2.tex defines them as colored for the marked-up version.)
\newcommand{\rev}[1]{#1}
\newcommand{\adj}[1]{#1}
\newcommand{\moved}[1]{#1}
...
Beyond reactive RF-only tracking, recent mmWave work has explored several proactive cues for beam management: sub-6~GHz channel features~\cite{Alrabeiah2020DeepLearning}, infrastructure-mounted LiDAR/radar/vision~\cite{Alkhateeb2023DeepSense, Jiang2023LiDAR, Morais2023Position}, the user device's current orientation~\cite{Rezaie2022Orientation, Struye2023CoVRage}, and bilateral reference-signal exchanges between access point and user device~\cite{Alkhateeb2018DLCB, Xue2023Standardization, Heng2021SixChallenges}. \rev{Machine-learning failure prediction on RF time-series features has been used to pre-emptively trigger a standard beam switch~\cite{9838772}; the 3GPP NR Beam Failure Recovery procedure~\cite{Giordani2018ATO, Heng2021SixChallenges} sends a discrete PRACH-based control message after the MAC entity counts $N$ consecutive Beam Failure Instances at L1; and sensor-aided V2X tracking~\cite{Brambilla_2019} streams IMU samples on every tracking interval over a separate out-of-band control channel.} Each has limitations: the sub-6~GHz approach needs a continuous lower-band reference; AP-side sensing adds infrastructure hardware and raises regulatory and privacy concerns; orientation-based methods select a beam from a precomputed list using only the present orientation, without anticipating when the beam is about to fail; reference-signal exchanges consume RF airtime continuously, even when the link is not at risk; \rev{RF-statistical failure prediction~\cite{9838772} cannot anticipate geometric edge crossings caused by rotation that have not yet manifested in RF; 3GPP NR Beam Failure Recovery is reactive (the recovery message fires after the link has already degraded); and continuous IMU streaming~\cite{Brambilla_2019} pays constant out-of-band overhead}. Most of these approaches were evaluated at mmWave, where beam residence times are an order of magnitude longer than in sub-THz. \emph{In this work we combine three elements specifically for sub-THz beam residence times: UE-side IMU motion sensing, a pre-failure trigger, and coordinated AP/UE realignment delivered through a single low-overhead control frame. To our knowledge, no prior approach jointly delivers all three over a sub-THz protocol with hardware-in-the-loop validation.}
...
The Electronic model (Fig.~\ref{fig:rot_simfast_rate}) also demonstrates that Hierarchical Search remains below the static reference, since fast steering cannot correct a misdirected search. IMU-Assist recovers to approximately \SI{38}{Gbps} at \SI{700}{deg/s^2}, while JIT sustains above \SI{57}{Gbps} across the full range by ensuring the AP holds position rather than scanning.
...
Across all four algorithms, the parameters reported in the subsections below were independently swept on a canonical translational pass-by ($D_{\text{initial}} = \SI{3}{m}$, $D_{\text{traversed}} = \SI{1.5}{m}$, peak acceleration $\SI{3}{m/s^2}$); the values listed in Table~\ref{tab:system_parameters} are the best-performing within $\pm 10\%$ of the optimum and are not specific to a single operating point. \rev{Table~\ref{tab:notation} summarises every symbol used in the equations below; numerical values for the parameters appear in Table~\ref{tab:system_parameters} (system parameters) and Table~\ref{tab:steering_and_hardware} (steering models).}

\rev{
\begin{table}[ht]
\centering
\small
\caption{Notation used throughout \emph{Methods} and \emph{Results}. Numerical values are listed in Table~\ref{tab:system_parameters} (system / algorithm parameters) and Table~\ref{tab:steering_and_hardware} (steering models).}
\label{tab:notation}
\sisetup{per-mode=symbol}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\linewidth}{@{}lX@{\hspace{10pt}}lX@{}}
\toprule
\textbf{Symbol} & \textbf{Description} & \textbf{Symbol} & \textbf{Description} \\
\midrule
\multicolumn{4}{@{}l}{\textbf{\textit{Coordinate frames, geometry, and kinematics}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$X_W, Y_W, Z_W$ & AP-anchored world-frame axes & $\mathbf{q}_{\text{rotary}}$ & Rotary-stage orientation quaternion \\
$\mathbf{x}_b, \mathbf{y}_b, \mathbf{z}_b$ & UE body-frame basis vectors & $\mathbf{q}_{\text{eff}}$ & Effective antenna orientation, $\mathbf{q}_{\text{body}} \otimes \mathbf{q}_{\text{rotary}}$ \\
$\mathbf{p}_k, \mathbf{q}_k$ & $k$-th keyframe position / orientation & $d_{\text{link}}$ & Instantaneous AP--UE LoS distance \\
$\mathbf{q}_{\text{body}}$ & UE body-frame orientation quaternion & $\hat{d}_{\text{link}}$ & Last known-good link distance estimate \\
$\mathbf{v}, \mathbf{v}_{\parallel}, \mathbf{v}_{\perp}$ & UE velocity; LoS-parallel; LoS-perpendicular & $\theta$ & Off-axis angle in antenna's local frame \\
$\boldsymbol{\omega}_{\text{UE}}$ & UE angular velocity (true) & $\theta_{\text{HPBW}}$ & Antenna half-power beamwidth \\
$\dot{\boldsymbol{\delta}}$ & Link Deviation Velocity, $\boldsymbol{\omega}_{\text{UE}} + \mathbf{v}_{\perp}/d_{\text{link}}$ & $\Delta\mathbf{p}(t)$ & Integrated UE displacement vector \\
$\Delta p_y, \Delta p_z$ & LoS-perpendicular displacement components & $\Delta\psi, \Delta\phi$ & Yaw and pitch changes since reference \\
$\tau, f(\tau)$ & Normalised segment time; Perlin smootherstep & $a_{\max}$ & Peak acceleration on a smootherstep segment \\
$\Theta$ & SLERP quaternion angle, $\arccos(\mathbf{q}_k\!\cdot\!\mathbf{q}_{k+1})$ & $D_{\text{initial}}, D_{\text{traversed}}$ & Canonical pass-by initial / traversed distance \\[2pt]

\multicolumn{4}{@{}l}{\textbf{\textit{RF channel and antenna}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$f_c, \lambda$ & Carrier frequency; wavelength $\lambda = c/f_c$ & $G_{\max}$ & Peak antenna gain, $\max_{\theta} G(\theta)$ \\
$B$ & Channel bandwidth & $G_{tx}(\theta), G_{rx}(\theta)$ & Tx / Rx directional gains \\
$T$ & System temperature & $\theta_{tx}, \theta_{rx}$ & Off-axis angle at Tx / Rx \\
$k_B T B$ & Thermal noise power & $L_p$ & Free-space path loss (dB) \\
$F$ & Rx noise factor & $P_{tx}, P_{rx}$ & Transmit / received power \\
$G(\theta)$ & E-plane antenna gain pattern of the horn & $\text{SNR}, \text{SNR}_{\text{th}}$ & Instantaneous SNR; 64-QAM threshold (\SI{18.8}{\deci\bel}) \\
$C(x), S(x)$ & Fresnel cosine / sine integrals & $M$ & QAM constellation order \\
$r_0, r_1, r_2$ & Fresnel-integral arguments for $G(\theta)$ & BER & Bit error rate (target $10^{-3}$) \\
$R_h, B_E$ & Horn slant length / E-plane aperture & $Q^{-1}(\cdot)$ & Inverse Gaussian Q-function \\
$a_{\text{wg}}, b_{\text{wg}}$ & Waveguide width / height & $\beta_g$ & Guide propagation constant \\[2pt]

\multicolumn{4}{@{}l}{\textbf{\textit{IMU sensor and bias model}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$n$ & Discrete IMU time-slot index & $\sigma_{a,n}$ & Accelerometer noise density \\
$\tilde{\mathbf{a}}[n], \tilde{\boldsymbol{\omega}}[n]$ & Raw accelerometer / gyroscope readings & $\sigma_{a,b}$ & Accelerometer bias random-walk std.\ dev.\ \\
$\mathbf{a}_{\text{corr}}[n], \boldsymbol{\omega}_{\text{corr}}[n]$ & Bias-corrected acceleration / angular velocity & $\sigma_p$ & Random-walk position drift over $T_{\text{react}}$ \\
$\mathbf{b}_a, \mathbf{b}_\omega$ & Accelerometer / gyroscope bias & $\mathbf{n}_a, \mathbf{w}_{b,a}$ & Accel.\ white noise; bias random-walk update \\
$\mathbf{f}_{\text{ideal}}[n]$ & Ground-truth (noise-free) acceleration & $T_{\text{IMU}}$ & IMU sampling period (\SI{10}{\milli\second}) \\[2pt]

\multicolumn{4}{@{}l}{\textbf{\textit{Beam-management timing}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$T_{\text{frame}}$ & Control-frame transmission time (\SI{5}{\micro\second}) & $T_{\text{react}}$ & Total JIT reaction time, $T_{\text{frame}} + T_{\text{proc}}$ \\
$T_{\text{proc}}$ & AP signal acquisition and processing delay & $\Delta t_{\text{sim}}$ & Simulation slot length (\SI{1}{\micro\second}) \\[2pt]

\multicolumn{4}{@{}l}{\textbf{\textit{Hierarchical Search parameters}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$\gamma$ & Spiral growth factor (1.25) & $\Gamma_{\text{fail}}$ & Failure margin below $P_{\text{ref}}$ \\
$HS_0, HS_k$ & Initial / $k$-th ring step size & $\Gamma_0, R$ & Initial success margin; margin-relaxation rate \\
$k$ & Spiral ring index & $P_{\text{succ}}(\Delta t)$ & Time-relaxing success threshold \\
$P_{\text{ref}}, P_{\text{fail}}$ & Reference / failure-detection power & $\Delta t$ & Elapsed time since failure detection \\[2pt]

\multicolumn{4}{@{}l}{\textbf{\textit{IMU-Assist and JIT parameters}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$\beta$ & IMU-Assist refinement step factor (0.10) & $\bar{\mathbf{a}}_{\text{predict},k}$ & EWMA-filtered acceleration trend (sample $k$) \\
$IA_k$ & IMU-Assist $k$-th refinement step size & $\mathbf{a}_k$ & Accelerometer sample at index $k$ \\
$\theta_{\text{az}}^{\star}, \theta_{\text{el}}^{\star}$ & IMU-Assist predicted azimuth / elevation & $\alpha_{\text{short}}$ & EWMA coefficient (0.90) \\
$\theta_{\text{az,0}}, \theta_{\text{el,0}}$ & Last known-good azimuth / elevation & $\eta$ & JIT guard fraction (0.36) \\[2pt]

\multicolumn{4}{@{}l}{\textbf{\textit{Steering hardware}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$\omega_{\max}, \alpha_{\max}$ & Stage max.\ angular velocity / acceleration & $\Delta\theta$ & Commanded angular displacement \\
$a(t)$ & Stage trapezoidal acceleration profile & & \\[2pt]

\multicolumn{4}{@{}l}{\textbf{\textit{Evaluation metrics (KPIs) and validation statistics}}} \\
\cmidrule(r){1-2} \cmidrule(l){3-4}
$C(t), \bar{C}$ & Instantaneous / mean Shannon capacity & $L_{\text{ctrl}}, N_{\text{pp}}$ & Foregone capacity; \# JIT control frames \\
$\sigma_C, \mu_C$ & Capacity std.\ dev.\ and mean & $\mathbbm{1}(\cdot)$ & Indicator function \\
CoV & Capacity coefficient of variation, $\sigma_C / \mu_C$ & MAPE & Mean absolute percentage error (sim vs HIL) \\
$A_{\text{link}}$ & Link availability fraction (\%) & NRMSE & Normalised root-mean-square error \\
$O_{\text{sig}}$ & Signaling overhead (\%) & $\bar{C}_{\text{sim},i}, \bar{C}_{\text{exp},i}$ & Sim.\ / HIL capacity at acceleration point $i$ \\
\bottomrule
\end{tabularx}
\end{table}
}
...
To verify that JIT's gains do not depend on a cherry-picked operating point and that the two baselines are fairly tuned, we sweep the most influential parameter of each algorithm and the IMU sampling period $T_{\text{IMU}}$. Each sweep is reported across the same translational ($0.5$--$20$~m/s$^2$) and rotational ($0.5$--$700$~deg/s$^2$) peak-acceleration grids used in \emph{Results}, following the Monte-Carlo and motion-randomization protocol of \emph{Statistics and Reproducibility}. The full acceleration sweep is drawn as faint background lines; the min, typical ($3$~m/s$^2$ / $100$~deg/s$^2$), and max accelerations are highlighted as bold lines. All sweeps use Ideal Mechanical steering at $d_{\text{link}} = \SI{3}{m}$, and the vertical dashed line on each panel marks the Table~\ref{tab:system_parameters} setting. Since $\gamma$, $\beta$, $\eta$ are dimensionless or normalized to $\theta_{\text{HPBW}}$, the parameter optima are invariant to HPBW; only the absolute capacity changes with antenna gain.

\begin{figure}[h!]
\centering
\input{Figures/RebFig_Tuning_HSIMU}
\caption{Baseline parameter sensitivity, Ideal Mechanical steering. Rows: translational (top), rotational (bottom). Columns: (a,c)~Hierarchical Search vs.\ $\gamma$; (b,d)~IMU-Assist vs.\ $\beta$. Bold lines mark the min, typical, and max accelerations. The chosen $\gamma = 1.25$ and $\beta = 0.10$ lie within $\pm 10\%$ of the peak at every acceleration shown.}
\label{fig:tuning_hsimu}
\end{figure}

\begin{figure}[h!]
\centering
\input{Figures/RebFig_Tuning_JITeta}
\caption{JIT guard fraction $\eta$ sensitivity, Ideal Mechanical steering. (a)~Translational; (b)~Rotational. Each panel overlays mean capacity (left axis) and signaling overhead (right axis). Mean capacity stays within $\pm 5\%$ of the maximum across the plateau $\eta \in [0.20, 0.50]$ at every acceleration shown.}
\label{fig:tuning_jiteta}
\end{figure}

\begin{figure}[h!]
\centering
\input{Figures/RebFig_Robustness_TimeScales}
\caption{Mean capacity vs.\ IMU sampling period $T_{\text{IMU}}$, Ideal Mechanical steering ($d_{\text{link}}=\SI{3}{m}$). (a)~Translational; (b)~Rotational. Bold lines mark the min, typical, and max accelerations; vertical dashed lines mark the Table~\ref{tab:system_parameters} baseline $T_{\text{IMU}} = \SI{10}{ms}$.}
\label{fig:tuning_timescales}
\end{figure}

The Hierarchical Search and IMU-Assist sweeps (Fig.~\ref{fig:tuning_hsimu}) confirm that the Table~\ref{tab:system_parameters} settings of $\gamma = 1.25$ and $\beta = 0.10$ sit at the peak of their sensitivity curves at every acceleration considered, so the comparison in \emph{Results} is not biased by a sub-optimal baseline choice. The JIT $\eta$ sweep (Fig.~\ref{fig:tuning_jiteta}) shows a wide plateau: across $\eta \in [0.20, 0.50]$, mean capacity stays within $\pm 5\%$ of the maximum and signaling overhead remains below $0.4\%$, so the choice $\eta = 0.36$ is one operating point on a flat region rather than a fragile tuning. The IMU sampling-period sweep (Fig.~\ref{fig:tuning_timescales}) confirms the sampling-rate decoupling discussed above: JIT's mean capacity stays within $3\%$ of its maximum across $T_{\text{IMU}} \in [1, 15]$\,ms, so $T_{\text{IMU}}$ is not the control-reaction bottleneck across the commodity range.
}
