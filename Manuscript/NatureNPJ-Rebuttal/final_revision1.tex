...
Maintaining beam alignment under mobility, commonly referred to as part of beam management, is a key bottleneck for mobile sub-THz networks\cite{Giordani2018ATO, 10422712, Wang2022}. Conventional \textbf{purely in-band} approaches used in modern networks, such as Kalman-filter-based tracking of Rx power, signal-to-noise ratio (SNR), bit error rate (BER), and other RF measurements\cite{8455289,9145366,10067030, 10123123}, demonstrate reliable performance for smooth, linear node motion but struggle with abrupt changes, such as unpredictable rapid device rotation by a human user\cite{Wang2022}. This limitation has recently motivated a shift toward developing new \textbf{out-of-band} strategies that also incorporate auxiliary sensor data into the beam management\cite{10494372, Azari2022, Nokia2022, Chen2023, Jornet2023}. Particularly, infrastructure-side sensors (radar, LiDAR, cameras) can track user position\cite{10422712,5GPPP_Automotive_Vision_2015}, while built-in on-device Inertial Measurement Units (IMUs) provide low-latency motion estimates directly at the user equipment (UE). Fusing IMU and RF data has been shown to outperform either source alone\cite{10036372,9391649,electronics8020142, s21082723, 9814507}. \emph{Still, the principal challenge remains, once the Tx and Rx sub-THz beams get misaligned, realigning them quickly in a mobile environment is difficult without an active Tx-Rx link, even with the use of auxiliary data (e.g., from IMUs).}

\rev{Beyond reactive RF-only tracking, recent work has explored proactive and out-of-band predictive beam management at mmWave, which we group into four families. \textbf{(i)}~Sub-6~GHz channel features have been used to anticipate beam and blockage decisions~\cite{Alrabeiah2020DeepLearning}, but require a continuous RF reference. \textbf{(ii)}~Infrastructure-side LiDAR, radar, vision, and GPS modalities have been integrated into multimodal predictors~\cite{Alkhateeb2023DeepSense, Jiang2023LiDAR, Morais2023Position}, adding AP-side hardware and raising non-trivial regulatory and privacy considerations (particularly for cameras), whereas IMUs are already integrated into most modern smartphones; GPS-aided variants further break down in the GPS-denied indoor environments that JIT explicitly targets. \textbf{(iii)}~On-device orientation has been used to condition UE-side beam selection in 3D~\cite{Rezaie2022Orientation} and head-mounted-display beamforming for mobile virtual reality~\cite{Struye2023CoVRage}, but as a codebook lookup from a current orientation snapshot rather than a time-projected pre-failure trigger over the system's reaction time. \textbf{(iv)}~Bilateral pilot-driven training within the 3GPP Rel-18 BM-Case framework~\cite{Alkhateeb2018DLCB, Xue2023Standardization, Heng2021SixChallenges} relies on continuous reference-symbol exchanges whose overhead scales with how often the pilots are sent; JIT, by contrast, fires a single event-triggered control frame only when a beam-edge crossing is imminent, holding total signaling overhead below 0.4\% of the channel capacity. Most of the approaches above were proposed and evaluated at mmWave, where beam residence times are an order of magnitude longer than in sub-THz; their direct applicability to truly mobile sub-THz links has not been comprehensively studied. \emph{Hence, in this work we combine three elements specifically for sub-THz beam residence times: UE-side IMU motion sensing, a pre-failure trigger, and coordinated AP/UE realignment, all delivered through a single low-overhead control frame. To our knowledge, no prior approach jointly delivers all three over a sub-THz protocol with hardware-in-the-loop validation.}}
...
Conventional millimeter-wave beam management strategies typically operate reactively, initiating recovery procedures only after signal degradation or link failure is detected\cite{Giordani2018ATO, Wang2022}. This approach is challenging in sub-THz networks due to the reliance on high-gain antennas\cite{Kleine-Ostmann2011}. While essential for closing the link budget, narrow beams create mobility constraints: they quadratically expand the spatial search space while shrinking the beam residence time (the window where a moving node stays aligned).
...
\moved{The key performance indicators when comparing these solutions include Mean Capacity, Capacity Coefficient of Variation (CoV), Link Availability, and Signaling Overhead (further discussed in Methods). Each group of results below illustrates the impact of different key input parameters: specifically, the separation distance between the nodes, the UE translational acceleration, UE rotational acceleration, directivity of the employed sub-THz antenna system, and various hardware constraints.}

\moved{Notably, for the latter, the performance levels for different beam alignment schemes (both the proposed JIT and the reference ones) are specifically evaluated across three representative beam steering models. These models reflect different types of steering mechanisms that can be possibly used in practical sub-THz communications (see Table~\ref{tab:steering_and_hardware}). Specifically, \textit{Constrained Mechanical} mirrors our laboratory testbed and anchors Hardware-in-the-Loop (HIL) validation; \textit{Ideal Mechanical} represents the upper bound of mechanical actuation for use cases such as backhaul and UAV links with high-speed rotation platforms; and \textit{Electronic} represents phased arrays envisioned for future mobile sub-THz access networks, extending state-of-the-art mmWave UE and AP phased antenna arrays to the sub-THz band.}

\rev{The four KPIs used throughout this section are formally derived in the \textit{Evaluation Metrics} subsection of \textit{Methods}: \emph{Mean Capacity} ($\bar{C}$) is the time-averaged Shannon capacity over the scenario duration; \emph{Capacity Coefficient of Variation (CoV)} is the normalized standard deviation of instantaneous capacity, capturing temporal stability; \emph{Link Availability} ($A_{\text{link}}$) is the fraction of time the SNR meets or exceeds the 64-QAM threshold (\SI{18.8}{\deci\bel}); and \emph{Signaling Overhead} ($O_{\text{sig}}$) is the channel capacity foregone by JIT control-frame transmissions, expressed as a percentage of the total link capacity.}
...
\rev{Notably, even with predictive coordination, a small residual gap persists between JIT and the Perfect Alignment reference visible in the steady-state portion of Fig.~\ref{fig:trans_metrics_example}. This residual reflects the finite end-to-end reaction latency $T_{\text{react}} \approx \SI{15}{\micro\second}$ during which the UE continues to drift after the AP issues its steering command. Eq.~(\ref{eq:trigger}) bounds this drift by $\eta \cdot \theta_{\text{HPBW}} = 0.36 \cdot \SI{13}{\degree} \approx \SI{4.7}{\degree}$ in the worst case (no prediction). In practice, JIT does not steer to the UE's last known position. Instead, the AP applies an EWMA filter to the IMU samples carried in the control frame, predicts where the UE will be when the steering command completes, and steers there. The actual residual is therefore much smaller than this worst-case bound, and the gain loss visible in Fig.~\ref{fig:trans_metrics_example} stays below \SI{1}{\deci\bel}, matching the small steady-state offset between JIT and Perfect Alignment. Trajectory information \emph{minimizes} but cannot eliminate this latency-induced residual on a non-zero-latency hardware platform.}
...
The Electronic model (Fig.~\ref{fig:rot_simfast_rate}) also demonstrates that Hierarchical Search remains below the static reference, since fast steering cannot correct a misdirected search. IMU-Assist benefits substantially, settling around \SI{38}{Gbps} at \SI{700}{deg/s^2}, while JIT sustains above \SI{57}{Gbps} across the full range by ensuring the AP holds position rather than scanning.
...
\end{equation}
where $\Gamma_{0}$ is the initial margin, $R$ is the margin increase rate, and $\Delta t$ is the elapsed time since failure detection. The relaxing margin accounts for the growing positional uncertainty: as more time elapses, the node accepts weaker beam configurations to avoid indefinite searching. Once a beam configuration exceeding $P_{\text{succ}}(\Delta t)$ is found, the node holds that beam direction and resumes data exchange. \rev{All Hierarchical Search parameters ($HS_0$, $\gamma$, $\Gamma_0$, $R$, $\alpha_{\text{short}}$) were independently swept on the canonical translational pass-by scenario used throughout the Results section ($D_{\text{initial}} = \SI{3}{m}$, $D_{\text{traversed}} = \SI{1.5}{m}$, peak acceleration $\SI{3}{m/s^2}$); the values reported in Table~\ref{tab:system_parameters} are the best-performing within $\pm 10\%$ of the optimum and are not specific to a single operating point.}
...
\end{equation}
Unlike the expanding step size $HS_k$ used in the Hierarchical Search's Search Phase, the base refinement step $\beta \cdot \theta_{\text{HPBW}}$ is fixed and deliberately small (one-tenth of the beamwidth), while the IMU-predicted azimuth and elevation corrections adapt $IA_k$ over time as the sensor estimates evolve. The node scans a tight spiral around the IMU-predicted direction using this step size until an alignment is made (see Fig.~\ref{fig:imu_local_search}). \rev{The refinement step factor $\beta$ and the IMU-prediction window length were similarly tuned on the same canonical translational pass-by ($D_{\text{initial}} = \SI{3}{m}$, $D_{\text{traversed}} = \SI{1.5}{m}$, peak $\SI{3}{m/s^2}$); the reported $\beta = 0.1$ minimizes overshooting at low acceleration while preserving fast convergence at high acceleration.}
...
We formally define the \emph{Edge of Coverage (EoC)} as the angle off the AP's beam axis at which the antenna gain has dropped by \SI{3}{\deci\bel}, i.e., $\theta_{\text{HPBW}}/2$ off boresight. Crossing the EoC corresponds to a worst-case \SI{6}{\deci\bel} round-trip gain loss and, depending on the link budget, can directly cause link failure. Because reaching the EoC under mobility leaves no time for control signaling, JIT instead places a \emph{trigger boundary} inside the EoC, at the offset where the projected displacement during the system reaction time $T_{\text{react}}$ would reach $\eta \cdot \theta_{\text{HPBW}}$. By construction, this boundary is \emph{adaptive}: as the UE's Link Deviation Velocity $\|\dot{\boldsymbol{\delta}}\|$ (defined below) grows, the projected displacement crosses $\eta \cdot \theta_{\text{HPBW}}$ at a smaller offset from boresight, and the trigger fires earlier, farther inside the EoC; conversely, when motion slows the boundary expands outward toward the EoC, suppressing unnecessary control frames. Importantly, for the AP-stationary scenarios considered in this work (e.g., a wall-mounted access point serving mobile UEs), detection of approaching EoC is performed \emph{locally} at the UE: at every IMU sample, the UE evaluates Eq.~(\ref{eq:trigger}) using its body-frame kinematics and the known $\theta_{\text{HPBW}}$ of its own antenna, requiring no AP-side feedback. Mobile-AP cases (e.g., UAV-borne access points) would require the AP to share its own kinematic state via the same control-frame protocol, a direct extension we leave to future work.}

For this purpose, since any practical IMU sensor features noise in its data, a stationary Zero Velocity Update (ZUPT) phase is performed at initialization to calibrate the accelerometer bias $\mathbf{b}_a$ and gyroscope bias $\mathbf{b}_\omega$. During this phase, the UE remains stationary for a brief period, and the mean of the raw accelerometer and gyroscope readings is computed. Since the true acceleration and angular velocity are both zero during this interval, the computed means directly estimate the sensor biases. These bias estimates are then subtracted from all subsequent IMU readings to obtain corrected acceleration $\mathbf{a}_{\text{corr}}[n] = \tilde{\mathbf{a}}[n] - \mathbf{b}_a$ and angular velocity $\boldsymbol{\omega}_{\text{corr}}[n] = \tilde{\boldsymbol{\omega}}[n] - \mathbf{b}_\omega$, enabling the system to distinguish link-critical motion from sensor noise. Utilizing the corrected IMU data, the UE then continuously computes a new metric, which we refer to as ``Link Deviation Velocity'', $\dot{\boldsymbol{\delta}}$. This metric unifies rotational and LoS-perpendicular translational UE motion into a single angular rate of deviation:
\begin{equation}
...
\end{equation}
The guard fraction $\eta$ trades off link robustness against control overhead in the JIT algorithm: a lower $\eta$ triggers control frames earlier, providing more margin against sudden motion at the cost of higher signaling frequency, while a higher $\eta$ reduces overhead but leaves less time to react before the beam exits the main lobe. We find $\eta = 0.36$ effective across our evaluated scenarios.

Upon triggering, the UE transmits a control frame containing its integrated displacement vector and a buffer of recent IMU samples. \rev{The AP derives a filtered acceleration trend $\bar{\mathbf{a}}_{\text{predict},k}$ from the buffered samples via an exponentially weighted moving average (EWMA),
\begin{equation}
...
Each control frame follows the IEEE~802.15.3d THz-SC structure\cite{9269931}, using a short Preamble Acquisition Preamble (PAP) of \SI{1.89}{\micro\second} and the minimum 2048-byte frame payload. With worst-case BPSK modulation and 11/15 LDPC coding, each control frame occupies approximately $T_{\text{frame}} \approx \SI{5}{\micro\second}$. The end-to-end reaction latency comprises the control frame transmission time ($T_{\text{frame}}$) and the AP's signal acquisition and processing delay ($T_{\text{proc}} = \SI{10}{\micro\second}$), giving a total signaling latency of $T_{\text{react}} = T_{\text{frame}} + T_{\text{proc}} \approx \SI{15}{\micro\second}$ before the AP can issue a steering command. Reliable decoding at the AP requires a minimum receive SNR of \SI{9.59}{\deci\bel}, corresponding to BPSK with 11/15 LDPC coding at a target $\text{BER} = 10^{-3}$. This threshold determines the maximum range at which the control frame remains decodable: if the instantaneous SNR falls below \SI{9.59}{\deci\bel}, the AP cannot decode the control frame and the system falls back to reactive recovery: the UE continues to steer using its local IMU data, while the AP, lacking motion awareness, initiates a Hierarchical Search to restore alignment.

\rev{\subsubsection*{IMU Drift and Sampling-Rate Considerations}
Two practical concerns naturally arise in IMU-driven beam management: integration drift accumulated since the last bias calibration, and the apparent disparity between the \SI{10}{\milli\second} IMU sampling period and the microsecond-scale control reaction. We address each in turn.

\textbf{First}, the trigger of Eq.~(\ref{eq:trigger}) is evaluated at every IMU sample, but each evaluation only projects the current Link Deviation Velocity forward by the reaction time $T_{\text{react}} \approx \SI{15}{\micro\second}$; the prediction horizon relevant to a single trigger decision is therefore $T_{\text{react}}$, not the full scenario duration. Modelling the accelerometer noise as a white process of one-sided density $\sigma_{a,n}$, the standard deviation of the position-prediction drift accumulated over this look-ahead follows the standard random-walk expression $\sigma_{p} = \sigma_{a,n}\sqrt{T_{\text{react}}^{3}/3}$. For the noise density listed in Table~\ref{tab:system_parameters} (\SI{0.0294}{m/s^2/\sqrt{\hertz}}) and $T_{\text{react}} \approx \SI{15}{\micro\second}$, this evaluates to $\sigma_{p} \lesssim \SI{1}{\nano\meter}$, corresponding to an angular drift of order $10^{-8}$~degrees at $d_{\text{link}} = \SI{3}{m}$, many orders of magnitude below $\theta_{\text{HPBW}}$. Two further design choices keep accumulated bias drift from corrupting the trigger across longer sessions: the predicate of Eq.~(\ref{eq:trigger}) uses the Link Deviation \emph{velocity} (single accelerometer integration, with drift growing linearly in time) rather than absolute position (double integration, quadratic in time); and the control-frame transmits the displacement \emph{delta since the last transmission} rather than an absolute position, with the accumulator reset at every transmission (cooldown $\approx \SI{50}{\milli\second}$), so the drift the AP ever sees is bounded by this short inter-transmission interval. The initial stationary ZUPT performed at startup remains necessary to calibrate the IMU bias before any motion is integrated; what becomes unnecessary at JIT's operating timescales is \emph{periodic re-ZUPT during continuous operation}, since the drift accumulated between bias updates over $T_{\text{react}}$ is negligible. For deployments with longer continuous mobility, opportunistic ZUPT events can be triggered whenever $\|\dot{\boldsymbol{\delta}}\|$ remains below a low-motion threshold for several control cycles, refreshing the bias estimate without interrupting data exchange.

\textbf{Second}, the \SI{10}{\milli\second} IMU sampling period gates only the \emph{evaluation rate} of Eq.~(\ref{eq:trigger}); it does not bound control reaction. Once a trigger fires, the control-frame transmission ($T_{\text{frame}} \approx \SI{5}{\micro\second}$), AP processing ($T_{\text{proc}} = \SI{10}{\micro\second}$), and electronic or mechanical actuation operate on their own clocks. Hence, the IMU rate caps the latency at which a new motion event is \emph{detected} (worst case, one IMU period), but does not affect how quickly the system \emph{responds} once detection has occurred. In our evaluation, this trigger-detection latency is small relative to the beam residence time at all considered motion levels, so the \SI{10}{\milli\second} sampling period does not become a bottleneck.}

\rev{\subsubsection*{Parameter Selection and Robustness}
The guard fraction $\eta = 0.36$ in Eq.~(\ref{eq:trigger}) was selected by sweeping $\eta \in [0.10, 0.72]$ on the canonical translational pass-by scenario: smaller $\eta$ provides earlier triggering at the cost of higher signaling rate, while larger $\eta$ leaves insufficient angular budget once the trigger fires. Mean capacity remains within \SI{5}{\percent} of its maximum across $\eta \in [0.25, 0.50]$, indicating gradual degradation around the chosen value. The reaction time $T_{\text{react}} \approx \SI{15}{\micro\second}$ is dictated by the control-frame and processing budgets and is therefore not freely tunable; reducing $T_{\text{react}}$ via shorter preamble or faster processing hardware would correspondingly relax the triggering condition and reduce signaling rate. The control-frame cooldown is set to one IMU sampling period to prevent redundant triggers within a single sensor cycle. Hence, the acceleration, distance, and beamwidth sweeps presented in the Results section also test how JIT performance varies as the operating conditions move away from this canonical scenario, complementing the explicit $\eta$ sweep summarized above.}
...
\end{equation}
where $\tau = (t - t_k) / (t_{k+1} - t_k) \in [0, 1]$ is the normalized time. This function's $C^2$ continuity guarantees zero velocity, acceleration, and jerk at keyframe boundaries, producing physically plausible motion. \rev{The zero-velocity boundary condition models pause-resume mobility patterns such as walking with intermediate stops or beginning and ending a turn from rest, which are the dominant motion classes considered in this study. Trajectories without a stationary boundary, such as continuous walking through multiple checkpoints, can be assembled by chaining smootherstep-interpolated segments whose endpoints exchange non-zero velocities, while preserving slot-by-slot $C^1$ continuity where required.} The resulting maximum acceleration for a displacement $D$ over duration $T$ is:
\begin{equation}
...
The second metric of interest in our study is the \emph{Capacity Coefficient of Variation (CoV)} used to quantify the capacity variance over time.
