...
Conventional millimeter-wave beam management strategies typically operate reactively, initiating recovery procedures only after signal degradation or link failure is detected\cite{Giordani2018ATO, Wang2022}. This approach is challenging in sub-THz networks due to the reliance on high-gain antennas\cite{Kleine-Ostmann2011}. While essential for closing the link budget, narrow beams \rev{(with half-power beamwidths, HPBW, on the order of a few degrees)} create mobility constraints: they quadratically expand the spatial search space while shrinking the beam residence time (the window where a moving node stays aligned).
...
The Electronic model (Fig.~\ref{fig:rot_simfast_rate}) also demonstrates that Hierarchical Search remains below the static reference, since fast steering cannot correct a misdirected search. IMU-Assist \rev{recovers to approximately} \SI{38}{Gbps} at \SI{700}{deg/s^2}, while JIT sustains above \SI{57}{Gbps} across the full range by ensuring the AP holds position rather than scanning.
...
%-------------------------------------------------------------------%
\subsection*{Impact of Antenna Directivity on Combined Motion}\label{sec:combined_motion}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.495\linewidth}
        ...
        \vspace{-25pt}
        \caption{Rotational}
        \label{fig:tuning_hs_rot}
    \end{subfigure}
    \caption{\rev{Hierarchical Search parameter sensitivity to the spiral growth factor $\gamma$ ($d_{\text{link}} = 3$~m, Ideal Mechanical). Bold lines mark the Lower, Typical, and Upper Acceleration values: $0.5$, $9.5$, and $20$ m/s$^2$ (translational); $0.5$, $300$, and $700$ deg/s$^2$ (rotational).}}
    \label{fig:tuning_hs}
    \vspace{-5pt}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.495\linewidth}
        ...
        \vspace{-25pt}
        \caption{Rotational}
        \label{fig:tuning_imu_rot}
    \end{subfigure}
    \caption{\rev{IMU-Assist parameter sensitivity to the refinement step factor $\beta$ ($d_{\text{link}} = 3$~m, Ideal Mechanical). Bold lines mark the Lower, Typical, and Upper Acceleration values: $0.5$, $9.5$, and $20$ m/s$^2$ (translational); $0.5$, $300$, and $700$ deg/s$^2$ (rotational).}}
    \label{fig:tuning_imu}
    \vspace{-5pt}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.495\linewidth}
        ...
        \vspace{-25pt}
        \caption{Translational}
        \label{fig:robustness_timescales_trans}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.495\linewidth}
        ...
        \vspace{-25pt}
        \caption{Rotational}
        \label{fig:robustness_timescales_rot}
    \end{subfigure}
